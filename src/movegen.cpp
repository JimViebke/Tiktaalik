...
namespace chess
{
	    ...
	    const size_t king_start_idx, const tt_key key, const bitboard from, const bitboard to)
	{
		...
		if (in_check || piece == king || move_type == move_type::en_passant_capture || from & blockers)
		{
			...
			if (child_board.in_check<moving_color, check_type>(king_idx)) return;
		}
	...
	force_inline_toggle static void find_pawn_moves(
	    size_t& end_idx, const board& parent_board, const bitboard blockers, const size_t king_idx, const tt_key key)
	{
			...
			if (parent_board.can_capture_ep())
			{
				...
				while (ep_capturers) // 0-2
				{
					...
					append_if_legal<moving_color, quiescing, perft, check_type, in_check, pawn,
					    ...
					    start, (1ull << ((moving_color == white) ? 16 : 40)) << ep_file);
				}
			}
		}
	...
	force_inline_toggle static void find_castle_moves(size_t& end_idx, const board& parent_board, tt_key key)
	{
		...
		if (can_castle_ks && (parent_bbs.occupied() & ks_castle_bits) == 0u)
		{
			...
			if (!parent_board.in_check<moving_color>(king_start_idx + 1))
			{
	...
	template <color moving_color, gen_moves gen_moves, bool quiescing, bool perft, check_type check_type, bool in_check,
	    piece piece>
	force_inline_toggle static void find_moves_for(
	    size_t& end_idx, const board& parent_board, const bitboard blockers, const size_t king_idx, const tt_key key)
	{
		...
		while (piece == king || pieces)
		{
			...
			while (captures && (gen_moves == gen_moves::captures || gen_moves == gen_moves::all))
			{
				...
				append_if_legal<moving_color, quiescing, perft, check_type, in_check, piece, move_type::capture>(
				    end_idx, parent_board, blockers, king_idx, incremental_key, from, to);
			}
	...
	template <color moving_color, gen_moves gen_moves, bool quiescing, bool perft, check_type check_type, bool in_check>
	force_inline_toggle static void find_moves(
	    size_t& end_idx, const board& parent_board, const bitboard blockers, const size_t king_idx, const tt_key key)
	{
		find_pawn_moves<moving_color, gen_moves, quiescing, perft, check_type, in_check>(
		    end_idx, parent_board, blockers, king_idx, key);
		...
		{
			find_castle_moves<moving_color, gen_moves, quiescing, perft>(end_idx, parent_board, key);
		}
	}
	...
	size_t generate_child_boards(const size_t parent_idx)
	{
		...
		if (last_moved_piece == pawn && square_is_attacked_by_pawn<opp_color>(parent_bbs, king_idx))
		{
			find_moves<moving_color, gen_moves, quiescing, perft, check_type::pawn, true>(
			    end_idx, parent_board, bitboard{}, king_idx, key);
		}
		...
		{
			if (parent_board.in_check<moving_color, check_type::sliders>(king_idx))
			{
